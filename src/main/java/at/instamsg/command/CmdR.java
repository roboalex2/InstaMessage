...
public class CmdR extends Command implements TabExecutor {
    ...
    public void execute(CommandSender sender, String[] args)
    {
        ...
        if(args.length < 1) {
            sender.sendMessage(new TextComponent(TextComponent.fromLegacyText(
                    "§7[§e!§7] §7Bitte beachte die Syntax: §c/r <Nachricht>")));
            return;
        }
        ...
        if(last == null) {
            sender.sendMessage(new TextComponent(TextComponent.fromLegacyText(
                    "§7[§e!§7] §cDu hast noch keinen Chat partner! " +
        ...
        if(target == null) {
            sender.sendMessage(new TextComponent(TextComponent.fromLegacyText(
                    "§7[§e!§7] §cDer Spieler ist nicht mehr online.")));
            return;
        }
        ...
        if(!targetData.canReceiveFrom(player)) {
            sender.sendMessage(new TextComponent(TextComponent.fromLegacyText(
                    "§7[§e!§7] §cDer Spieler hat Privatnachrichten " +
        ...
        target.sendMessage(new TextComponent(TextComponent.fromLegacyText(
                String.format("§e[§6%s §e» §cmir§e]§7 %s",
                player.getDisplayName(), message))));
        sender.sendMessage(new TextComponent(TextComponent.fromLegacyText(
                String.format("§e[§cich §e» §6%s§e]§7 %s",
